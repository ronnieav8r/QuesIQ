...
export default function HistoryScreen() {
  ...
  return <Screen bottomInset={false} eyebrow="Your progress" subtitle="Saved transcripts and coaching reviews. QuesIQ does not retain raw session audio." title="History" refreshControl={<RefreshControl colors={[colors.cyan]} onRefresh={() => history.refetch()} refreshing={history.isRefetching} tintColor={colors.cyan} />}>
    <Pressable accessibilityRole="button" accessibilityLabel="View progress" onPress={() => router.push("/progress")} style={styles.progressLink}><Text style={styles.loadMoreText}>View progress</Text><ArrowRight color={colors.cyan} size={16} /></Pressable>
...
const styles = StyleSheet.create({ emptyBody: { color: colors.muted, fontSize: 14, lineHeight: 21 }, emptyTitle: { color: colors.text, fontSize: 18, fontWeight: "800" }, loadMore: { alignItems: "center", borderColor: colors.border, borderRadius: 12, borderWidth: 1, justifyContent: "center", minHeight: 48, padding: spacing.sm }, loadMoreText: { color: colors.cyan, fontSize: 15, fontWeight: "800" }, meta: { flexDirection: "row", flexWrap: "wrap", gap: spacing.md }, metaItem: { alignItems: "center", flexDirection: "row", gap: spacing.xs }, metaText: { color: colors.muted, fontSize: 13 }, progressLink: { alignItems: "center", flexDirection: "row", gap: spacing.xs, minHeight: 44 }, role: { color: colors.textSoft, fontSize: 14, lineHeight: 20 }, status: { alignItems: "center", alignSelf: "flex-start", backgroundColor: colors.panelStrong, borderRadius: 99, flexDirection: "row", gap: spacing.xs, paddingHorizontal: spacing.sm, paddingVertical: spacing.xs }, statusComplete: { backgroundColor: colors.limeDark }, statusText: { color: colors.muted, fontSize: 12, fontWeight: "800" }, statusTextComplete: { color: colors.lime } });
