...
export default function HistoryScreen() {
  ...
  return <Screen eyebrow="Your progress" subtitle="Saved transcripts and coaching reviews. QuesIQ does not retain raw session audio." title="History" refreshControl={<RefreshControl colors={[colors.cyan]} onRefresh={() => history.refetch()} refreshing={history.isRefetching} tintColor={colors.cyan} />}>
    ...
    {history.hasNextPage ? <Pressable accessibilityLabel="Load more saved reviews" disabled={history.isFetchingNextPage} onPress={() => history.fetchNextPage()} style={styles.loadMore}><Text style={styles.loadMoreText}>{history.isFetchingNextPage ? "Loading more…" : "Load more reviews"}</Text></Pressable> : null}
  </Screen>;
}
