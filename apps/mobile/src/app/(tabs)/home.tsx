...
export default function HomeScreen() {
  ...
  const { catalog, jobTargets, profile, sessions, user } = bootstrap.data;
  const target = jobTargets.find((item) => item.id === preferredTargetId(bootstrap.data));
  const firstImpression = catalog.practiceModes.find((mode) => mode.key === "first_impression" && (mode as { enabled?: boolean }).enabled !== false);
  const latest = sessions.find((session) => session.hasEvaluation && session.evaluation);
  const score = latest?.evaluation ? average(latest.evaluation.scores) : undefined;

  return <Screen bottomInset={false} eyebrow="QuesIQ Interview" keyboardAvoiding subtitle="One focused conversation at a time." title={`Ready, ${profile?.preferredName || user.name || "there"}?`}>
    {firstImpression ? <Card accent="cyan">
    ...
    <Card onPress={() => router.push("/(tabs)/me")} title="Active target" trailing={<BriefcaseBusiness color={colors.lime} size={20} />}>
      <Text style={styles.cardTitle}>{target?.targetRole || profile?.targetRole || "Add a role to practice for"}</Text>
      <Text style={styles.body}>{target?.targetCompany || profile?.targetCompany || "Your practice will become more specific once a target is added."}</Text><Text style={styles.link}>Edit target in Me <ArrowRight color={colors.cyan} size={15} /></Text>
    </Card>
    <View style={styles.metrics}>
      <Card accent="lime" style={styles.metricCard}><Target color={colors.lime} size={22} /><Text style={styles.metric}>{score === undefined ? "—" : `${score}%`}</Text><Text style={styles.caption}>Latest score</Text></Card>
      <Card style={styles.metricCard}><Text style={styles.metric}>{sessions.length}</Text><Text style={styles.caption}>Recent sessions</Text></Card>
    </View>
...
const styles = StyleSheet.create({
  ...
  kicker: { color: colors.cyan, fontSize: 11, fontWeight: "900", letterSpacing: 1.2 }, link: { alignItems: "center", color: colors.cyan, flexDirection: "row", fontSize: 13, fontWeight: "800" }, metric: { color: colors.text, fontSize: 28, fontWeight: "900" }, metricCard: { flexBasis: 150, flexGrow: 1, minWidth: 140 }, metrics: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }, row: { alignItems: "flex-start", flexDirection: "row", gap: spacing.md },
});
