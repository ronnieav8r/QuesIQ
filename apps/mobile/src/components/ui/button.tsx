...
export function Button({ disabled, icon: Icon, label, loading, onPress, variant = "primary" }: ButtonProps) {
  ...
  return (
    <Pressable accessibilityRole="button" accessibilityState={{ disabled: Boolean(inactive), busy: Boolean(loading) }} disabled={inactive} onPress={onPress} style={({ pressed }) => [styles.base, styles[variant], pressed && !inactive && styles.pressed, inactive && styles.disabled]}>
...
const styles = StyleSheet.create({
  base: { alignItems: "center", borderRadius: radius.md, flexDirection: "row", gap: spacing.sm, justifyContent: "center", minHeight: layout.primaryHeight, paddingHorizontal: spacing.md, paddingVertical: spacing.sm },
  danger: { backgroundColor: "transparent", borderColor: colors.danger, borderWidth: 1 },
  disabled: { opacity: 0.45 },
  label: { color: colors.text, flexShrink: 1, textAlign: "center", fontSize: 16, fontWeight: "800" },
