...
export function buildInterviewExecutionConfig(input: InterviewExecutionConfigInput): InterviewExecutionConfig {
  ...
  if (configured.modeKey === "coaching") {
    set("engine", "turn_based", "Coaching uses the native chained execution engine.");
    set("textModel", "gpt-5.4-mini", "Coaching native chain text model.");
    set("transcriptionModel", "gpt-live-transcribe", "Coaching native chain transcription model.");
    set("ttsModel", "gpt-4o-mini-tts", "Coaching native chain speech model.");
    set("ttsVoice", "marin", "Coaching native chain voice.");
  } else {
  ...
  const revision = createHash("sha256").update(stable({ surface: input.surface, configured, effective, promptVersions })).digest("hex");
